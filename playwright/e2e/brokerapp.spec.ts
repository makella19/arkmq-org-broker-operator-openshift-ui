...
test.describe('BrokerApp lifecycle', () => {
  test.beforeAll(() => {
    ...
    // Verify the PKI secrets were actually created in the operator namespace.
    console.log('[DIAG] Cert/CA secrets in operator namespace (default):');
    console.log(
      kubectl('get secrets -n default --no-headers | grep -E "manager|cert|ca"', {
        ignoreError: true,
      }) || '  (none matched)',
    );
    ...
    // Verify the service cert and CA bundle were distributed to the test namespace.
    console.log(`[DIAG] Secrets in test namespace ${TEST_NAMESPACE} after service cert creation:`);
    console.log(
      kubectl(`get secrets -n ${TEST_NAMESPACE} --no-headers`, { ignoreError: true }) || '  (none)',
    );
  ...
  test.afterAll(() => {
    // ── Diagnostic dump captured before cleanup so CI artifacts contain root-cause info ──
    console.log('\n========= DIAGNOSTIC DUMP (pre-cleanup) =========');

    console.log(`\n[DIAG] Pods in ${TEST_NAMESPACE}:`);
    console.log(kubectl(`get pods -n ${TEST_NAMESPACE} -o wide`, { ignoreError: true }));

    console.log(`\n[DIAG] StatefulSets in ${TEST_NAMESPACE}:`);
    console.log(kubectl(`describe statefulsets -n ${TEST_NAMESPACE}`, { ignoreError: true }));

    console.log(
      `\n[DIAG] Describe pod ${SERVICE_NAME}-ss-0 (may not exist if operator never created it):`,
    );
    console.log(
      kubectl(`describe pod ${SERVICE_NAME}-ss-0 -n ${TEST_NAMESPACE}`, { ignoreError: true }),
    );

    console.log(`\n[DIAG] Events in ${TEST_NAMESPACE} (sorted by time):`);
    console.log(
      kubectl(`get events -n ${TEST_NAMESPACE} --sort-by=.lastTimestamp`, { ignoreError: true }),
    );

    console.log('\n[DIAG] BrokerService conditions:');
    console.log(
      kubectl(
        `get brokerservice ${SERVICE_NAME} -n ${TEST_NAMESPACE} -o jsonpath='{.status.conditions}' 2>/dev/null || echo '(not found)'`,
        { ignoreError: true },
      ),
    );

    console.log('\n[DIAG] Operator logs (last 200 lines):');
    console.log(
      kubectl('logs -n default deployment/arkmq-org-broker-controller-manager --tail=200', {
        ignoreError: true,
        timeout: 90000,
      }),
    );
