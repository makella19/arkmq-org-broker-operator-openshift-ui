...
describe('ConnectionInformationSection', () => {
  ...
  it('shows the placeholder alert when the connectivity test button is clicked', async () => {
    render(<ConnectionInformationSection service={service} />);
    await userEvent.click(screen.getByTestId('broker-app-run-connectivity-test'));
    expect(screen.getByTestId('broker-app-connectivity-result')).toBeInTheDocument();
  });
});
